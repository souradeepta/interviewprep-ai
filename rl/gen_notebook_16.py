"""Generator for notebook 16: Model-Based RL (Dyna-Q)"""
import nbformat
import json
...
c3 = md(
    ...
    "GridWorld: 5x5 grid, agent starts at (0,0), goal at (4,4). "
    "Actions: 0=up, 1=down, 2=left, 3=right. "
    "Reward: +1 at goal, -0.01 per step, walls block movement. "
    "Compare K=0 (pure Q-learning) vs K=5 vs K=20 planning steps per real step.\n"
)
...
c5 = md(
    ...
    "Train a neural dynamics model f_theta(s, a) -> (r, s') using sklearn MLPRegressor. "
    "Use it for planning: generate K simulated transitions, update Q-table. "
    "Track model prediction error (MSE) over training.\n"
)
...
c7 = md(
    ...
    "Compare real-environment steps needed to reach a performance threshold "
    "for model-free (K=0), Dyna-Q (K=5, K=20), and neural model-based approaches. "
    "Run multiple seeds for stable estimates.\n"
)
...
c9 = md(
    ...
    "Demonstrate how imperfect world model errors compound over long rollout horizons. "
    "Short rollouts (1-3 steps) from model stay near real experience. "
    "Long rollouts (10-20 steps) accumulate compounding prediction error.\n"
)
...
c11 = md(
    ...
    "Encode GridWorld state into a compact latent representation (PCA). "
    "Train world model in latent space - this mirrors how DreamerV2/RSSM works at scale. "
    "Evaluate 1-step prediction accuracy of the latent dynamics model.\n"
)
...
c15 = md(
    "## Key Takeaways\n\n"
    "**Core idea:** Dyna-Q uses a learned model M(s,a) -> (r, s') to generate K simulated transitions "
    "after each real step, improving sample efficiency proportional to K without additional real interactions.\n\n"
...
c16 = md(
    ...
    "2. **Model accuracy vs K**: Collect only 100 random transitions before training. "
    "Does K=20 still outperform K=0?\n"
    "3. **Stochastic environment**: Add 20% random action noise to GridWorld. "
    "How does this affect the deterministic model accuracy?\n"
    "4. **Latent dimension sweep**: Retrain latent model with latent_dim=2 vs 8. "
    "How does state reconstruction accuracy change?\n"
)
...
out = '/home/sbisw/github/interviewprep-ml/rl/notebooks/16-model-based-rl.ipynb'
