"""Generator for notebook 18: Inverse RL"""
import nbformat
...
c15 = md(
    "## Key Takeaways\n\n"
    "**Core idea:** IRL infers the reward function R that rationalizes expert behavior, "
    "then trains a new policy on the inferred R. BC skips reward inference; GAIL approximates "
    "IRL without solving the inner RL problem.\n\n"
...
c16 = md(
    "## Exercises\n\n"
    "1. **BC horizon limit**: In `evaluate_bc_over_horizon`, at what horizon H does "
    "the BC policy first fail to beat random? What's the inflection point?\n"
    "2. **IRL inner loop depth**: In `run_maxent_irl`, reduce `n_iter` in "
    "`softmax_value_iteration` to 5 (from 20). How does recovered reward quality change?\n"
    "3. **Noisy expert BC**: Generate demos with noise=0.3 (30% random actions). "
    "Does BC performance degrade? Does IRL degrade as much?\n"
    "4. **GAIL without RL**: Remove Q-learning from GAIL, keep only the discriminator reward. "
    "Show that GAIL needs RL to work; discrimination alone is insufficient.\n"
)
...
out = '/home/sbisw/github/interviewprep-ml/rl/notebooks/18-inverse-rl.ipynb'
